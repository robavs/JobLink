...
const inptus = [
    {
        title: "Grad", property: "city", uniqueCheck: false, textTransform: "capitalize"
    },
    {
        title: "Adresa", property: "address", uniqueCheck: false, textTransform: "capitalize"
    }
]

export const ThirdPage = () => {
    const { inputValues, inputStatus, setInputValues, setInputStatus, canGoToNextPage } = useRegistration()
    const { country, city, address } = inputStatus


    useEffect(() => {
        canGoToNextPage(country, city, address)
    }, [inputValues])

    return (
        <div className="form-group mb-3">
            <div>
                {/*Ova Lista država je preuzeta sa interneta*/}
                <label htmlFor="country" className="form-label">Država:</label>
                <select value={inputValues["country"]}
                    onChange={e => {
                        setInputStatus(values => ({ ...values, [e.target.id]: "valid" }))
                        setInputValues(values => ({ ...values, [e.target.id]: e.target.value }))
                    }}

                    className="form-select" id="country" name="country" required>
